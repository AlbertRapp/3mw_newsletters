...
ui <- fluidPage(
  actionButton(
    inputId = 'my_cool_btn',
    label = 'Click me!',
    style = 'border-radius: 10px; width: 200px; font-weight: bolder;margin-top:10px;'
  )
)
...
my_python_button = ui.input_action_button(
  id = 'my_cool_btn',
  label = 'Click me!',
  style = 'border-radius: 10px; height: 50px; width: 100px; font-weight: bold;padding:0px;margin-top:10px;'
)
...
def server(input, output, session):
  @output
  @render.text
  @reactive.event(input.my_cool_btn)
  def text_output():
    return input.text_input() + ' ' + str(input.numeric_input())
```
